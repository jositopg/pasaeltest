...
function AuthScreen({ onLogin }) {
  ...
  const MVP_MODE = true; // Cambiar a false para auth real
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 🚀 MVP MODE: Login automático sin validación
      if (MVP_MODE) {
        const mvpUser = {
          id: isLogin ? `mvp-user-${formData.email}` : `user-${Date.now()}`,
          email: formData.email || 'demo@opositia.com',
          password: formData.password,
          name: formData.name || formData.email?.split('@')[0] || 'Usuario Demo',
          oposicion: formData.oposicion || 'Guardia Civil',
          createdAt: new Date().toISOString(),
          subscription: 'free',
          isGuest: false,
          isFirstLogin: !isLogin // Onboarding solo en registro
        };
        
        // Guardar en storage para persistencia
        await storage.set('current-user', mvpUser);
        await storage.set('user-session', { userId: mvpUser.id, loginAt: new Date().toISOString() });
      ...
      // 🔒 MODO PRODUCCIÓN: Auth real con validación
      if (isLogin) {
        // Login
        ...
        await storage.set('user-session', { userId: user.id, loginAt: new Date().toISOString() });
        onLogin(user);
      } else {
        // Registro
        const users = await storage.get('users') || [];
        ...
        await storage.set('user-session', { userId: newUser.id, loginAt: new Date().toISOString() });
          ...
          {MVP_MODE && (
            <div className="bg-green-500/20 border border-green-500/40 rounded-xl p-3 mb-4">
              <p className="text-green-300 text-xs text-center font-semibold">
                🚀 MODO MVP: Login simplificado - Cualquier email/password funciona
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-gray-300 text-sm mb-2">
                Email {MVP_MODE && <span className="text-gray-500 text-xs">(opcional en MVP)</span>}
              </label>
              <input
                type="email"
                required={!MVP_MODE}
            ...
            <div>
              <label className="block text-gray-300 text-sm mb-2">
                Contraseña {MVP_MODE && <span className="text-gray-500 text-xs">(opcional en MVP)</span>}
              </label>
              <input
                type="password"
                required={!MVP_MODE}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Cargar temas (usar numThemes del perfil)
      const numThemes = loadedProfile.numThemes || 90;
      const storageKey = `oposit-themes-${currentUser.id}`;
      const t = await storage.get(storageKey) || 
        Array.from({ length: numThemes }, (_, i) => ({ 
          number: i + 1, 
          name: `Tema ${i + 1}`, 
          documents: [], 
          questions: [] 
      ...
      const historyKey = `oposit-history-${currentUser.id}`;
      const h = await storage.get(historyKey) || [];
      
      setThemes(t);
      setExamHistory(h);
      
      if (!await storage.get(storageKey)) {
        await storage.set(storageKey, t);
      }
  ...
  const updateTheme = async (theme) => {
    ...
    setThemes(newThemes);
    await storage.set(`oposit-themes-${currentUser.id}`, newThemes);
  };
  ...
  const finishExam = async (score) => {
    ...
    await storage.set(`oposit-history-${currentUser.id}`, newHistory);
    setScreen('home');
  };
